...
timeHud()
{
	...
	{
		case "owner": self.timerHud[6] setText("^5Owner"); break;
		case "masteradmin": self.timerHud[6] setText("^9Master Admin"); break;
		case "adminplus": self.timerHud[6] setText("^1Admin+"); break;
		case "admin": self.timerHud[6] setText("^6Admin"); break;
		case "member": self.timerHud[6] setText("^3Member"); break;
		default: break;
	}
...
updatewrHud()
{
	...
	for(i = 0; i < level.normal_way.size; i++)
	{
		if(way == "ns"+i && speed == 190)
		{
			...
			self.timerHud[3] setText("(WR)                 ^2"+ 
				level.normal_way_info_190[i][0]["time"].min + 
				":"+
				level.normal_way_info_190[i][0]["time"].sec + 
				"."+
				level.normal_way_info_190[i][0]["time"].milsec
			);
		}	
		...
		if(way == "ns"+i && speed == 210)
		{
			...
			self.timerHud[3] setText("(WR)                 ^2"+ 
				level.normal_way_info_210[i][0]["time"].min + 
				":"+
				level.normal_way_info_210[i][0]["time"].sec + 
				"."+
				level.normal_way_info_210[i][0]["time"].milsec
			);
		}
	}
	...
	for(i = 0; i < level.secret_way.size; i++)
	{
		if(way == "s"+i && speed == 190)
		{
			...
			self.timerHud[3] setText("(WR)                 ^2"+ 
				level.secret_way_info_190[i][0]["time"].min + 
				":"+
				level.secret_way_info_190[i][0]["time"].sec + 
				"."+
				level.secret_way_info_190[i][0]["time"].milsec
			);
		}	
		...
		if(way == "s"+i && speed == 210)
		{
			...
			self.timerHud[3] setText("(WR)                 ^2"+ 
				level.secret_way_info_210[i][0]["time"].min + 
				":"+
				level.secret_way_info_210[i][0]["time"].sec + 
				"."+
				level.secret_way_info_210[i][0]["time"].milsec
			);
		}
	}
}
	...
	for(i = 0; i < level.normal_way.size; i++)
	{
		if(way == "ns"+i && speed == 190)
		{
			...
			if(self.pb_190["ns_"+i].size > 0)
				self.timerHud[2] setText("(PB)                  ^3"+
					self.pb_190["ns_"+i]["time"].min+
					":"+
					self.pb_190["ns_"+i]["time"].sec+
					"."+
					self.pb_190["ns_"+i]["time"].milsec
				);
		}
		...
		if(way == "ns"+i && speed == 210)
		{
			...
			if(self.pb_210["ns_"+i].size > 0)
				self.timerHud[2] setText("(PB)                  ^3"+
					self.pb_210["ns_"+i]["time"].min+
					":"+
					self.pb_210["ns_"+i]["time"].sec+
					"."+
					self.pb_210["ns_"+i]["time"].milsec
				);
		}
	}
	...
	for(i = 0; i < level.secret_way.size; i++)
	{
		...
		if(way == "s"+i && speed == 190)
		{
			...
			if(self.pb_190["s_"+i].size > 0)
				self.timerHud[2] setText("(PB)                  ^3"+
					self.pb_190["s_"+i]["time"].min+
					":"+
					self.pb_190["s_"+i]["time"].sec+
					"."+
					self.pb_190["s_"+i]["time"].milsec
				);
		}
		...
		if(way == "s"+i && speed == 210)
		{
			...
			if(self.pb_210["s_"+i].size > 0)
				self.timerHud[2] setText("(PB)                  ^3"+
					self.pb_210["s_"+i]["time"].min+
					":"+
					self.pb_210["s_"+i]["time"].sec+
					"."+
					self.pb_210["s_"+i]["time"].milsec
				);
		}
	}
}
