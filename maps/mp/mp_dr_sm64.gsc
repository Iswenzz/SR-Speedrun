...
main()
{
	...
	thread sr\api\_map::create_spawn((-5963, -7623, 1452), 357);
	thread sr\api\_map::create_normal_way("Normal Way;");
	thread sr\api\_map::create_secret_way("^5Slide Secret;^1Hard Secret;");

	eztp = getEnt("secret_slide_tp","targetname");
	thread sr\api\_map::create_tp((-5992, -8082, 1452), 100, 150, 
		eztp.origin, eztp.angles[1], "freeze", "cyan", "s0");
	htp = getEnt("hardsecret_enter_tp","targetname");
	thread sr\api\_map::create_tp((-5748, -8070, 1452), 100, 150, 
		htp.origin, htp.angles[1], "freeze", "darkred", "s1");
	//thread sr\api\_map::create_secret_way("Slide Secret;Hard Secret;"); // TODO
	...
	// thread star3set();
	thread bluecoin();
	thread doorclose(); // TODO
	thread startsound();
	thread warpsetup();
	thread castlesound();
	thread lvl1sound();
	thread beachng();
	thread secretsetup(); // TODO
	thread lvl1();
	thread tplvl1();
	thread tplvl1fx();
	// thread mapzor();
